...
const getDayHeatmap = (tasks: any[]) => {
  ...
  if (ratio === 1)
    return { bg: "bg-green-100 dark:bg-green-950/40", border: "border-green-400", text: "text-green-700" };
  if (ratio >= 0.66)
    return { bg: "bg-lime-100 dark:bg-lime-950/40", border: "border-lime-400", text: "text-lime-700" };
  if (ratio >= 0.33)
    return { bg: "bg-amber-100 dark:bg-amber-950/40", border: "border-amber-400", text: "text-amber-700" };
  if (ratio > 0)
    return { bg: "bg-orange-100 dark:bg-orange-950/40", border: "border-orange-400", text: "text-orange-700" };
  return { bg: "bg-rose-50 dark:bg-rose-950/30", border: "border-rose-300", text: "text-rose-600" };
};
...
export const CalendarTab = ({ project }: CalendarTabProps) => {
  ...
  const getTasksForDay = (day: number, month = current.month, year = current.year) => {
  ...
  return (
    <div className="bg-card rounded-xl border border-gray-100 shadow min-h-[calc(100vh-300px)] p-4 space-y-4">
      {/* Header */}
              ...
              {day && (
                <>
                  ...
                  <div className="space-y-0.5">
                    ...
                    {dayTasks.length > 2 && (
                      <div className={`text-[10px] pl-0.5 font-medium ${heatmap.text || "text-muted-foreground"}`}>
        ...
        <div className="border border-border rounded-xl p-4 space-y-3 bg-muted/30 animate-in fade-in slide-in-from-bottom-2 duration-200">
          ...
          <div className="h-1.5 rounded-full bg-muted overflow-hidden">
            <div
              ...
              style={{ width: `${(completedCount / selectedTasks.length) * 100}%` }}
            />
          </div>
